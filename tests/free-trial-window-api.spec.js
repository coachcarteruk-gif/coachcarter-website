...
test.describe('free trial API booking window', () => {
  test('picker requests 28 days and renders a slot on the boundary', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: /Select 10:00/ })).toBeVisible();
    await expect(page.locator('.date-cell-open')).toHaveAttribute('data-date', isoDaysFromToday(28));
