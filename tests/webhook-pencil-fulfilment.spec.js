...
function fulfilmentHarness({ status = 'pending', calendar = [] } = {}) {
  ...
  const client = {
    async query(text, args = []) {
      ...
      if (sql.startsWith('INSERT INTO credit_transactions')) {
        state.feeWrites.credit = args[5];
        state.creditTransactions += 1;
        return { rows: [{ id: 801 }], rowCount: 1 };
      }
      if (sql.startsWith('INSERT INTO lesson_bookings')) {
        state.feeWrites.booking = args[11];
        state.feeWrites.bookingSource = args[12];
        state.bookings += 1;
        return { rows: [{ id: 901 }], rowCount: 1 };
      }
      if (sql.startsWith('INSERT INTO booking_credit_sources')) {
        state.feeWrites.attribution = args[6];
        return { rows: [], rowCount: 1 };
      }
  ...
  const run = (overrides = {}) => fulfilPencilledOffer({
    ...
    amountPence: 5500,
    fundingEvidence: { feePence: overrides.feePence === undefined ? 170 : overrides.feePence },
...
test.describe('pencilled-offer webhook fulfilment transaction', () => {
  for (const feePence of [null, 0, 170]) {
    test(`preserves actual fee evidence (${feePence}) across booking and funding rows`, async () => {
      const harness = fulfilmentHarness();
      expect(await harness.run({ feePence })).toMatchObject({ applied: true });
      expect(harness.state.feeWrites).toEqual({
        credit: feePence, booking: feePence, attribution: feePence,
        bookingSource: feePence === null ? null : 'balance_transaction',
      });
    });
  }
