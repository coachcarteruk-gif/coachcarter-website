...
test.describe('instructor add lesson payment links', () => {
  ...
  test('calendar payment-link branch posts minimal existing-learner offer payload', () => {
    ...
    const offerBranch = sliceBetween(body, "if (isPaymentLink) {", "const result = await BookingActions.postWithScheduleCheck(\n      '/api/instructor?action=create-booking'");

    expect(offerBranch).toContain("BookingActions.postWithScheduleCheck(\n        '/api/instructor?action=create-offer'");
  ...
  test('cash, credit, and free still use the existing create-booking payloads', () => {
    ...
    expect(indexBody).toContain("BookingActions.postWithScheduleCheck(\n      '/api/instructor?action=create-booking'");
    ...
    expect(dashboardBody).toContain("BookingActions.postWithScheduleCheck(\n      '/api/instructor?action=create-booking'");
