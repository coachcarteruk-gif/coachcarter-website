...
**Booking:** 12-week (84-day) advance cap for self-serve. Slot-first UX on `book.html` (no calendar views, no lesson-type pill bar — pick slot first, then duration in modal). Offer-driven recurring series (`lesson_offers.max_repeat_weeks` 1–18) is the only path that may exceed the 12-week cap. Existing lessons can receive a paid or explicitly free extension through `create-extension-offer`; native clients must treat this as a mutation of the original booking, not a second adjacent booking. Paid extensions fulfil after verified payment; free extensions fulfil directly on learner acceptance without changing credit or payable value.
...
| `offers.js` | public (token) | get-offer, accept-offer (with `repeat_weeks`, or booking-bound extension metadata), expire-offers (cron). Exports `bookOfferSeries()` for the webhook |
...
- **Booking extension offers (September 2026)** — `lesson_offers.extension_booking_id`, `extension_minutes`, and `extension_base_list_price_pence` identify a paid addition to one scheduled booking. Acceptance reuses the token/Checkout screens; the webhook atomically adds funding/BCS evidence and updates that booking. Mobile clients should render `is_extension` from `get-offer` and reuse the server APIs rather than reproducing pricing or clash rules.
- **Availability enforcement (21 September 2026)** — Native booking/edit flows must treat `409 SCHEDULE_UNAVAILABLE` as a hard refusal and send users to availability management, never retry with `availability_override`. Schedule changes may return `409 AVAILABILITY_BOOKING_CONFLICTS` with lesson details and `availability_conflict_token`; display every affected lesson and resubmit the exact proposal/token only after acknowledgement. `public/shared/instructor-availability-review.js` is the web client adapter. Existing bookings remain unchanged.
...
**Notable additions (July 2026):**
- **Lesson requests (request-to-book)** — `lesson_requests` table + `instructors.request_to_book` toggle + new route file `api/requests.js` (expire cron) + shared module `api/_lesson-requests.js`. Learner actions on `api/slots.js` (`request-slot`, `checkout-request`, `my-requests`, `withdraw-request`), instructor decisions on `api/instructor.js` (`list-requests`, `accept-request`, `decline-request`), webhook `payment_type='lesson_request_hold'`. Payment held not taken: Stripe `capture_method='manual'` for cards; `request_hold`/`request_refund` `credit_transactions` types for credits. Pending requests block their slot everywhere pending offers do. **App port implication:** native booking screens must branch on the slot feed's `request_to_book` flag ("Request this slot" vs instant book) and reuse these endpoints; the status strings (`pending`/`accepted`/`declined`/`expired`/`withdrawn`) are API contract. See `LESSON-REQUEST-PLAN.md`.
- **Payout v2 ledger foundation (inactive, July 2026)** — additive append-only funding-source, booking-earning, batch, transfer, adjustment, Stripe-event, and connected-bank-payout tables are defined in `db/migrations/035_payout_v2_ledger_foundation.sql`, with pure funding/fingerprint contracts in `api/_payout-v2-contracts.js`. Every v2 row requires an explicit `school_id`; legacy/pre-Connect funding is structurally zero-payable; one booking has one earning across direct/school routes; transfer and bank-payout states are distinct. `schools.payout_engine_version` remains `v1`, so this foundation changes no live API behaviour. **App port implication:** a future native instructor payout screen must consume the eventual server planner/read model and its versioned states; it must never reproduce funding classification, live-price fallback, source-cap, fingerprint, or payout calculations client-side. See `docs/payout-v2-implementation-plan.md`.
- **Payout v2 source ingestion foundation (inactive, July 2026)** — new successful credit purchases and direct slot payments now have an additive server-side path to immutable `payout_funding_sources`, deduplicated both by school-scoped Stripe event receipt and by a school/credit-transaction/source fingerprint. Positive Stripe value requires PaymentIntent, charge, balance transaction, collected amount/currency, and fee evidence; incomplete or contradictory evidence is zero-value `manual_review`, and legacy ingestion is forced to zero payable value. The read-only preview requires an explicit `PAYOUT_V2_SCHOOL_ID`. No schema deployment, source backfill, payout activation, or transfer is part of this slice. **App port implication:** PaymentSheet success is not accounting authority. Native clients must keep sending the server-priced metadata contract including explicit `school_id`/`instructor_id`, then rely on the signed webhook and server source writer; they must not classify funding or calculate payout value locally.
- **Payout v2 reviewed historical ingestion and producer coverage (inactive, July 2026)** — the local `import:payout-v2-sources` operator tool defaults to a read-only, explicitly school-scoped dry-run. Any future apply requires an independent environment gate, exact confirmation phrase, expected candidate count/totals, and the exact reviewed SHA-256 plan fingerprint; the candidate snapshot is rechecked inside the transaction. `payout_source_import_runs`, immutable sources, and the audit record are append-only and atomic. Paid lesson-offer Checkout success and captured request-to-book PaymentIntent success now use the same inactive source writer with explicit tenant metadata and retryable event receipts. Missing Stripe evidence remains zero-payable `manual_review`, and legacy remains forced to zero; cash/Setmore/external payments and live prices are never inferred. No import or migration has been applied. **App port implication:** native offer/request success screens must treat server webhook completion as accounting authority and must never create, classify, or value payout sources locally.
- **Payout v2 future-payout recovery foundation (inactive, July 2026)** — the owner-approved £414 legacy duplicate is modelled as an append-only negative recovery obligation, with positive child applications conserved against `payout_batches.recovery_deducted_pence`. The pure full-offset planner withholds up to the whole otherwise-payable future Fraser amount, floors the transfer at zero, and carries any remainder forward. Creation requires exact same-school completed-v1-payout and legacy-line evidence; no opening adjustment has been inserted and the v1 payout path is unchanged. **App port implication:** future payout statements must display gross entitlement, recovery deducted, net transfer, and remaining recovery from the server read model. A native client must never calculate, apply, waive, or mutate a recovery locally.
- **Payout v2 source-backed earning shadow (inactive, July 2026)** — one pure server planner now calculates versioned/fingerprinted `chargeable` booking earnings exclusively from immutable funding-source allocations, enforces exact pence conservation/source caps/one-route ownership, and fails closed for legacy, missing, manual-review, and cross-scope evidence. A school-scoped shadow read model compares v2 with the current v1 preview while keeping v1 live-price fallback out of v2 inputs. The transaction-only materializer can write planned v2 earning/batch/claim/recovery-application rows after fingerprint revalidation, but no admin mutation route, transfer row, Stripe call, migration deployment, source import, or activation change exists. By owner decision, vehicle deposits are entirely off-system: v2 fingerprints that policy, always deducts/records zero, and identifies the current v1 £195/£250 heuristic only as a deliberate comparison difference. **App port implication:** native payout views may render the eventual server shadow/statement response, including blockers and deliberate v1 differences, but must never calculate earnings, choose a funding class, infer or collect deposits, materialise claims, or initiate transfers client-side.
- **Payout v2 durable transfer executor/reconciler (inactive, July 2026)** — Materialized batches retain the exact reviewed plan snapshot, and the server-only executor revalidates that snapshot, claims, earnings, source allocations/caps, recovery, destination, route exclusivity, totals, calculation version, and plan fingerprint before persisting source-linked transfer intents. Stripe charge groups use `source_transaction`; logical fingerprints and Stripe idempotency keys are deterministic; append-only attempt evidence preserves ambiguous outcomes. `submitting` never falls through to claim deletion, timeouts become `reconciling`, confirmed no-transfer evidence becomes `failed_confirmed`, and same-day reconciliation can attach a lost Stripe success or distinguish safe same-key retry from operator review. Zero-value batches skip Stripe. The module requires an injected Stripe client and is not imported by any live API, admin action, cron, webhook, or native surface; migration 035 is installed schema-only, but no import, activation, or transfer execution has occurred. **App port implication:** native clients must never call this executor or reproduce its transfer grouping/idempotency logic. They should eventually consume a server read model that distinguishes planned/claimed/submitting/reconciling/Connect-transferred from bank-paid/bank-failed, and show structured blocker/operator-review reasons without offering a client-side retry that invents a new key.
- **Payout v2 signed webhook and bank-payout visibility (inactive, July 2026)** — The internal handler verifies raw-body Stripe signatures before any I/O, deduplicates supported connected-account transfer/payout/refund/dispute events through durable receipts, derives tenant scope from an immutable globally unique connected-account registry, and stores only minimised append-only evidence. Connected bank payouts correlate to local transfers only through exact payout balance-transaction source identities; approximate amount/date matching is forbidden. Out-of-order or contradictory terminal evidence becomes operator review, and a failed bank payout never erases the earlier successful Connect transfer. Migration 035 is installed schema-only; no live connected-bank webhook route or Stripe client is wired, and v1 remains active. **App port implication:** native payout screens must consume the future server read model and preserve its distinct copy for Connect-transferred, bank-paid, bank-payout-failed, and operator-review states. They must never verify webhooks, infer bank settlement from transfer creation, correlate payouts locally, or expose a client-side payout/retry mutation.
- **Payout v2 protected balance and operator controls (inactive, July 2026)** — One server authority calculates protected free cash from Stripe available cash minus exact source-attributed unused learner exposure, disjoint instructor/refund obligations, and an append-only configured reserve; pending cash and transfer readiness remain separate. Global and school scopes are explicit, and an undivided platform Stripe balance cannot be presented as school free cash. A fingerprint-bound withdrawal preflight, cron/superadmin/scoped-operator guard, awaited deduplicated alert contract, append-only evidence tables, read-only diagnostic, and manual Dashboard runbook are implemented without a live route, cron, UI, alert transport, or Stripe mutation. **App port implication:** native clients may eventually render the versioned server calculation, blockers, preflight decision, and operator wording, but must never read Stripe secrets, calculate liquidity, choose a reserve, approve a withdrawal, infer school cash allocation, or create a payout locally. Every attempted use must recheck the server fingerprint.
- **Payout v2 controlled-cutover preparation (inactive, July 2026)** — Additive append-only config, two-cycle shadow, readiness, transition/reconciliation/incident evidence plus pure server contracts now make route, first-live instructor/hard cap, reserve/protected fingerprint, external/cash/Setmore classification, owner approval, and named operator explicit per school. The future transition primitive atomically rechecks one `v1` school and immutable evidence before recording the event and setting only that school to `v2`; it is not imported by a route. The dormant v1 guard refuses instructor and school payout mutation only after a future school actually becomes `v2`, before claim/write/Stripe I/O. The capped dry-run cannot truncate a plan or call Stripe, and rollback preserves claims/evidence while reconciliation continues. Migration 035 is installed schema-only and inactive; the source-ingestion application rollout is prepared but not approved or deployed, and all schools remain v1. **App port implication:** native clients may display version-labelled readiness, blockers, transfer, bank-payout, and incident states from a future server read model, but must never approve cutover, choose a cap/route/reserve/operator, alter fingerprints, initiate a transfer, infer bank settlement, or implement rollback locally.
