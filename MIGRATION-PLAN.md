...
**Booking:** 12-week (84-day) advance cap for self-serve. Slot-first UX on `book.html` (no calendar views, no lesson-type pill bar — pick slot first, then duration in modal). Offer-driven recurring series (`lesson_offers.max_repeat_weeks` 1–18) is the only path that may exceed the 12-week cap. Existing lessons can receive a paid extension through `create-extension-offer`; native clients must treat this as a verified-payment mutation of the original booking, not a second adjacent booking.
