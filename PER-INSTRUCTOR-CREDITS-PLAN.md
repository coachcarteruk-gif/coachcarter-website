...
**Plan B3 (2026-05-21):** `/api/migrate-step-2c-no-lcb-backfill` handles the Group C residual that Plan A and Plan B1 didn't reach — pre-cutover learners with **no LCB row at any instructor** but active booking draws funded from the legacy pooled balance (`instructor_id IS NULL` CTs the cron excludes from its purchases CTE). For each `(learner, instructor)` pair where (a) no LCB exists anywhere for the learner, (b) no per-pair CT exists, and (c) active draws > 0 (using the cron's `booking_draws` predicate exactly), B3 inserts ONE synthetic `legacy_grandfather` CT with `minutes = SUM(active draws at the pair)`. **No LCB writes** — these learners spent their entire legacy pool, there's no remaining balance to grandfather. The math: `actual_lcb = 0`, `expected = draws − draws = 0`, `drift = 0` by construction. The "no LCB anywhere" gate deliberately excludes the Simon cross-instructor case (learner=11 has LCB at (11, 4) from B1), preserving it as visible drift for manual resolution. Marker `per_instructor_credits_step_2c_no_lcb_backfill`; prereq is the B1 marker.
