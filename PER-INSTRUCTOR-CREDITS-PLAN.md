...
**Plan B3 (2026-05-21):** `/api/migrate-step-2c-no-lcb-backfill` handles the Group C residual that Plan A and Plan B1 didn't reach — pre-cutover learners with **no LCB row at any instructor** but active booking draws funded from the legacy pooled balance (`instructor_id IS NULL` CTs the cron excludes from its purchases CTE). For each `(learner, instructor)` pair where (a) no LCB exists anywhere for the learner, (b) no per-pair CT exists, and (c) **clean** active draws > 0 — where "clean" applies the cron's `booking_draws` predicate **plus** `status != 'refunded'` — B3 inserts ONE synthetic `legacy_grandfather` CT with `minutes = SUM(clean draws at the pair)`. **No LCB writes** — these learners spent their entire legacy pool, there's no remaining balance to grandfather. The math: `actual_lcb = 0`, `expected = clean_draws − total_draws = -bug_draws`, `drift = +bug_draws` — refund-bug residual stays visible to the cron as chip #3's target rather than being silenced under `legacy_grandfather`. Marker `per_instructor_credits_step_2c_no_lcb_backfill`; prereq is the B1 marker.
