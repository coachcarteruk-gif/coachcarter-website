...
New instructor bookings, fixed/broadcast offers, time edits and extension offers must fit recurring or explicit one-off availability. `availability_override` and `force` must never bypass normal hours, busy blocks, blackouts or external-calendar events. Intentional exceptions are made by changing availability first. Upcoming admin time edits follow the same rule; historical corrections and existing payment settlement contracts remain supported. Availability reductions show newly uncovered existing lessons for acknowledgement and leave those bookings intact. See `docs/instructor-availability-policy.md`.
