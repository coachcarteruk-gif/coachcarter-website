...
New instructor bookings, fixed/broadcast offers, time edits and extension offers must fit recurring or explicit one-off availability. Exceptions: instructor-created Flexible Hours bookings and lesson extension requests may override normal hours after a warning and explicit confirmation using a lesson-specific `normal_hours_override_token`. Busy blocks, blackouts and external-calendar events remain blocking. Legacy `availability_override` and `force` inputs do not bypass these checks. Other intentional exceptions require changing availability first. Upcoming admin time edits follow the same rule; historical corrections and existing payment settlement contracts remain supported. Availability reductions show newly uncovered existing lessons for acknowledgement and leave those bookings intact. See `docs/instructor-availability-policy.md`.
