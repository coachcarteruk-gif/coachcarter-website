...
-- ============================================================
-- Migration 053: 10-hour Flexible Hours package
-- ============================================================
...
ALTER TABLE flexible_package_purchases
  ...
  CHECK (product_slug IN ('flexible-10-hours', 'flexible-15-hours', 'flexible-30-hours'));
-- Booked-lesson curriculum progress live beta (migration 054).
CREATE UNIQUE INDEX IF NOT EXISTS uq_lesson_bookings_id_school ON lesson_bookings(id, school_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_driving_sessions_id_school ON driving_sessions(id, school_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_driving_sessions_booking ON driving_sessions(booking_id) WHERE booking_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS uq_learner_users_id_school ON learner_users(id, school_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_instructors_id_school ON instructors(id, school_id);
...
CREATE INDEX IF NOT EXISTS idx_curriculum_submissions_booking_role ON curriculum_review_submissions(school_id, booking_id, assessor_role, submitted_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_curriculum_submissions_learner ON curriculum_review_submissions(school_id, learner_id, submitted_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_curriculum_submissions_instructor ON curriculum_review_submissions(school_id, instructor_id, submitted_at DESC, id DESC);
...
CREATE INDEX IF NOT EXISTS idx_curriculum_ratings_latest ON curriculum_rating_events(school_id, learner_id, curriculum_item_key, assessor_role, assessed_at DESC, id DESC);
CREATE INDEX IF NOT EXISTS idx_curriculum_ratings_booking ON curriculum_rating_events(school_id, booking_id, assessed_at DESC, id DESC);
...
CREATE INDEX IF NOT EXISTS idx_curriculum_completions_learner ON curriculum_completion_events(school_id, learner_id, completed_at DESC);

-- Curriculum progress runtime-role grants (migration 055).
DO $$
DECLARE
  runtime_role RECORD;
BEGIN
...
-- Interim v1 manual-settlement handoff boundary (migration 057).
-- Additive and inert: a boundary is recorded only by the separately confirmed
-- superadmin route. It does not create a payout, approval or transfer.
...
CREATE INDEX IF NOT EXISTS idx_interim_v1_manual_boundary_scope
  ON interim_v1_manual_settlement_boundaries(school_id, instructor_id, settled_before_at);
  ...
  FOR runtime_role IN
    SELECT DISTINCT r.rolname
     ...
     WHERE c.relname = 'interim_v1_instructor_controls'
       ...
       AND r.rolcanlogin
  LOOP
    EXECUTE format('GRANT SELECT, INSERT ON TABLE public.interim_v1_manual_settlement_boundaries TO %I', runtime_role.rolname);
  END LOOP;
END $$;


-- Legacy school-wide hours conversion (058).
...
-- September 2026: paid extensions attached to an existing lesson.
...
-- Migration 061: authoritative payout evidence (additive and inert).
...
CREATE TABLE IF NOT EXISTS payout_funding_basis_events (
  ...
  CHECK (value_semantics IN ('gross_customer_revenue','net_after_processing','final_instructor_payable')),
  ...
  CHECK (
    ...
    OR (value_semantics = 'net_after_processing'
    ...
    OR (value_semantics = 'final_instructor_payable'
...
DROP TRIGGER IF EXISTS payout_flexible_source_evidence_append_only ON payout_flexible_source_evidence;
CREATE TRIGGER payout_flexible_source_evidence_append_only
  BEFORE UPDATE OR DELETE ON payout_flexible_source_evidence
  FOR EACH ROW EXECUTE FUNCTION interim_v1_forbid_append_only_change();
DROP TRIGGER IF EXISTS payout_direct_evidence_observations_append_only ON payout_direct_evidence_observations;
CREATE TRIGGER payout_direct_evidence_observations_append_only
  ...
  FOR EACH ROW EXECUTE FUNCTION interim_v1_forbid_append_only_change();
DROP TRIGGER IF EXISTS payout_funding_basis_events_append_only ON payout_funding_basis_events;
