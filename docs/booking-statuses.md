...
The payout authority considers a booking only when `status = 'chargeable'`; this includes a delivered lesson and a late learner cancellation which deliberately remains payable under the 48-hour rule. It excludes `refunded`, test and already-claimed bookings. Simon's controlled first period additionally classifies by the Europe/London lesson-end instant in its stored half-open boundary. Chargeable status is necessary but not sufficient: missing or contradictory immutable funding evidence blocks reconciliation rather than producing a zero earning. No extra grace period is added — the 1-hour buffer on `scheduled → chargeable` already absorbs clock skew and last-minute reschedule races, and there is no confirmation step that could stall.
