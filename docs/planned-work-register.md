...
| Refunds | Manual refund evidence, notes, discovery, and incident-repair polish | partially shipped | 82% | P2 | medium | The safe ledger path exists, manual-bank records capture evidence metadata, refund events have searchable admin discovery/detail, notes preserve incident context, and incident readiness can classify existing events before any repair mutation exists. | `docs/refund-operator-runbook.md:30-33`, `api/_refund-manual-bank.js`, `api/_refund-events.js`, `api/_refund-notes.js`, `api/_refund-incident-readiness.js`, `public/admin/portal.js`; records store bank references/evidence, event discovery is school-scoped/read-only, notes support operator/evidence/incident/repair-decision entries, add-note actions are audit-logged, and readiness returns `complete` / `incomplete` / `needs_manual_decision` without Stripe calls or refund/booking/payout/credit mutation. | Build the separately reviewed repair mutation for Stripe-success/local-ledger-failure cases, using the readiness design and stop conditions in the runbook. |
