...
| Refunds | Manual refund evidence, notes, discovery, and incident-repair polish | partially shipped | 75% | P2 | medium | The safe ledger path exists, manual-bank records capture evidence metadata, refund events have searchable admin discovery/detail, and notes preserve incident context; true repair remains intentionally manual. | `docs/refund-operator-runbook.md:30-33`, `api/_refund-manual-bank.js`, `api/_refund-events.js`, `api/_refund-notes.js`, `public/admin/portal.js`; records store bank references/evidence, event discovery is school-scoped/read-only, notes support operator/evidence/incident/repair-decision entries, and add-note actions are audit-logged. | Add a dedicated repair workflow for Stripe-success/local-ledger-failure cases. |
