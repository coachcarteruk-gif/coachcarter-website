...
| Refunds | Manual refund evidence, notes, and incident-repair polish | partially shipped | 65% | P2 | medium | The safe ledger path exists, manual-bank records capture basic evidence metadata, and refund events now have an admin notes timeline; true repair remains intentionally manual. | `docs/refund-operator-runbook.md:30-32`, `api/_refund-manual-bank.js`, `api/_refund-notes.js`, `public/admin/portal.js`; records store bank references/evidence, notes support operator/evidence/incident/repair-decision entries, and add-note actions are audit-logged. | Add refund-event search/discovery and a dedicated repair workflow for Stripe-success/local-ledger-failure cases. |
