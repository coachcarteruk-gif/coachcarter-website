...
The instructor dimension is introduced only when actual package units are allocated to a booking. The booking stores the delivering same-school active `instructor_id`, the sum of its immutable source contributions in `list_price_pence`, `list_price_source='flexible_package_frozen_rate'`, and zero *per-booking transaction* Stripe fee because Stripe processed the package only once. Payout calculation must therefore allocate the original package transaction's actual fee across immutable 30-minute units; the zero booking field is not evidence that the package had no fee. Existing scheduled/chargeable/refunded payout eligibility then applies normally. Entitlement creation alone has no instructor and creates no earning or payout entry.
