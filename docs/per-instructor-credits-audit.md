...
| Social-video booking discount | `api/slots.js`, `api/webhook.js`, `api/_pricing-helpers.js` | Instructor opt-in + learner per-booking consent + 18+ confirmation | Yes | `social_video_consent` and `social_video_age_confirmed` are separate request booleans. The server validates `instructors.social_video_opt_in` and requires the 18+ confirmation before applying the 5% discounted Stripe amount / charge minutes, then snapshots consent, age confirmation, and discount pct on `lesson_bookings`. |
...
| Learner credit booking | `api/slots.js` credit transaction path | LCB + FIFO `credit_transactions` by instructor | Yes | Locks/decrements chosen instructor row, writes BCS and list price. If the instructor has opted in to social-video filming and the learner consents with 18+ confirmation, the lesson blocks the full duration but deducts the server-calculated 5%-discounted charge minutes and snapshots the filming fields on `lesson_bookings`. |
...
| Cancellations | `api/slots.js`, `api/instructor.js` cancel paths | Booking `instructor_id` + `lockBalanceAdjustLCB` | Mostly yes | Returns credit to original instructor row and marks BCS refunded. Self-serve free-trial learner cancellations terminate the booking without credit mutation because `minutes_deducted=0`. Not fully transactional with booking update. |
...
| Learner booking UI | `public/learner/book.js` | Selected slot instructor balance | Mostly yes | Booking modal reads `/api/credits?action=balance&instructor_id=<slot instructor>` before showing credit eligibility and uses `selected_instructor_balance_minutes`. When social-video filming is available, eligibility and price copy use discounted charge minutes only after the learner consents and confirms they are 18+. Aggregate balance remains elsewhere. |
