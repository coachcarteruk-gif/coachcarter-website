...
| Reschedule | `api/slots.js`, `api/instructor.js` reschedule paths | Same-instructor moves carry original BCS; learner instructor switches write paired transfer-out/transfer-in CT rows and replacement BCS | Yes | Same-instructor moves do not mutate balances. Ordinary learner switches preserve the paid entitlement without a second charge, move its instructor scope atomically, and keep the immutable original Stripe source linked through `transferred_from_credit_transaction_id`. Reserved Weekly Slot moves remain same-instructor only. |
