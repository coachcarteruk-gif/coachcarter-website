...
- An ordinary learner reschedule may switch to another active same-school instructor. The replacement booking becomes the delivery and Friday-payout booking. Its original Stripe source remains immutable; paired `instructor_transfer_out` / `instructor_transfer_in` credit rows move the consumed entitlement and the replacement BCS points at the new instructor-scoped row. Any later eligible cancellation therefore returns credit with the replacement instructor. Reserved Weekly Slot moves remain same-instructor only.
