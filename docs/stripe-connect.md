...
- `api/_authoritative-lesson-earning.js` — source-backed per-lesson earning authority used by Simon's controlled preview, instructor payout-period preview, calendar-week lesson lines, admin overview, approval, and processor
...
- Before payout, instructors can remove their own past unpaid lesson from eligibility with `POST /api/instructor?action=mark-not-delivered`; the endpoint refuses bookings already present in `payout_line_items`.
...
- The legacy selector now uses active BCS contribution or `lesson_bookings.list_price_pence` only; it never recomputes a historical payout from current lesson-type/custom pricing. Missing gross or fee evidence raises `PAYOUT_FUNDING_RECONCILIATION_REQUIRED` before payout creation or Stripe transfer. Flexible Hours are deliberately blocked from that selector and use the source-aware controlled authority.
...
- Customer refund wording must distinguish credit returns from approved cash/card/bank-payment refunds. 48+ hour lesson cancellations return lesson credit to the learner's balance with the booking instructor; they are not automatic cash refunds. Where a cash or original-payment-method refund is approved as an exception, it should be returned to the original payment method where possible, minus the original Stripe processing fee because Stripe does not return processing fees from the original transaction. Partial refunds follow the same net-of-fee rule and Stripe refunds normally take 5-10 days to appear on the customer's account. This wording should be solicitor-reviewed before being treated as final legal copy.
- An ordinary learner reschedule, or an instructor-managed reschedule of their own lesson, may switch to another active eligible same-school instructor. The replacement booking becomes the delivery and Friday-payout booking. Its original Stripe source remains immutable; paired `instructor_transfer_out` / `instructor_transfer_in` credit rows move the consumed entitlement and the replacement BCS points at the new instructor-scoped row. Any later eligible cancellation therefore returns credit with the replacement instructor. The learner is not charged again. Reserved Weekly Slot moves remain same-instructor only.
- Bank-paid Reserved Weekly Slot v1 follows the same default cancellation value rule: eligible 48+ hour learner cancellations return same-instructor Lesson Credit, not automatic cash or Stripe refunds. Cash/original-payment-method refunds remain admin/operator exceptions. Stripe Dashboard confirms Pay by Bank refund support and partial refund support, original-method-only return, 5-10 day refund timing, and non-returned original Stripe processing fees. If an approved Stripe original-method refund fails, cannot be funded from the Stripe balance, cannot return to the original method, or is otherwise blocked, Fraser handles the refund manually by bank transfer as the last resort and records it through `POST /api/admin?action=record-manual-bank-refund`.

## Current Simon launch direction — hardened interim v1

Migration 043 and the application changes described in [`simon-interim-v1-hardening-implementation.md`](simon-interim-v1-hardening-implementation.md) now implement this boundary for review. They have not been deployed or operated by the implementation milestone. Account preparation is durable and ambiguity-safe; it establishes `payouts_start_date` and `payouts_paused=true` before a provider call, while invitation remains a separate command. A controlled instructor is excluded from the generic cron/admin payout path and cannot be unpaused by the generic control.

The owner preview and guarded first-run path require immutable funding evidence inside the recorded half-open payout interval. Direct Stripe lessons require the exact live Checkout/PaymentIntent/Charge/balance-transaction chain, actual gross, actual fee, and funds-availability fact. Flexible Hours allocate the original package gross and its one actual processing fee deterministically over the package's 30-minute units. Manual, cash, bank, external, correction, and legacy value is positive only through an append-only audited funding-basis event with explicit `gross_customer_revenue`, `net_after_processing`, or `final_instructor_payable` semantics. Unclassified or incomplete evidence is a visible blocker, never a zero earning. Approval and movement are separate fingerprint-bound superadmin commands; ambiguous provider outcomes retain claims for same-identity reconciliation, success leaves the instructor paused, and a completed first run cannot authorize a second under this milestone.

The 13 August 2026 owner decision keeps CoachCarter school-wide on payout engine v1 while Simon is onboarded and initially paid through a hardened, human-controlled Express/v1 route. This supersedes the earlier requirement to complete Accounts v2/payout v2 before Simon could be paid; it does not declare the current v1 routes safe without changes.

The implemented review boundary makes account creation tenant-scoped, audit-logged and recoverable after ambiguous success; deliberately sets `payouts_start_date`; starts the controlled instructor with `payouts_paused=true`; and provides Fraser a read-only preview of exact lessons, source evidence, fees, blockers and proposed transfer. `chargeable` remains required. Pre-boundary, after-period, refunded, claimed and test bookings are excluded. Missing evidence, unclassified legacy value and contradictory allocation duration remain reconciliation blockers rather than being presented as £0.

For a commission instructor, the per-lesson rule is `round((attributable customer revenue - actual attributable processing fee) × commission rate)`. The processing fee is deducted exactly once before the share. A direct payment owns its actual fee. A package owns one actual fee which is penny-conserved across its immutable units; lessons consume the value and fee of their units. A genuinely non-Stripe payment has no Stripe fee unless audited evidence proves a real processing fee. `final_instructor_payable` evidence bypasses commission multiplication so an inherited net/final value cannot be charged commission twice.
...
The retained Accounts v2 test account shell, intents, attempts and A8 evidence must be preserved, inactive and separate from any Production v1 identity. The Accounts v2 and payout-v2 sections below are long-term, inactive/deferred work; their reconciliation, two shadow Fridays, controlled cutover and four live approvals are not prerequisites for Simon's interim v1 route.

## Refund Preview / Execute Policy

Approved card, wallet, and Pay by Bank refunds are net of the original non-refundable Stripe processing fee. The learner absorbs that original processing fee; no goodwill exception is assumed by default.
...
Paid lesson extensions append a separate Stripe-backed `slot_purchase` and `booking_credit_sources` row to the existing lesson. The booking's minutes and payable list price increase only after verified payment succeeds, so normal chargeable/payout calculation sees one longer lesson with combined value and exact per-source fee evidence. Open extension Checkout sessions are expired when the offer or source booking is invalidated. If payment completes concurrently and the extension can no longer be fulfilled, the webhook creates a durable, idempotent `booking_extension_unfulfilled` refund event and returns the full extension charge; an unsuccessful provider call is retained as `manual_review` and alerted. This narrow compensation does not authorize any other automatic refund path or payout-engine change.

An explicitly free lesson extension bypasses Stripe. Learner acceptance changes the existing booking's end time and accepts the offer atomically, but does not change `minutes_deducted`, list price, BCS attribution, refundable value or instructor payout value.
...
Failed or blocked Stripe original-method refunds follow the same last-resort manual bank policy. Do not retry with a different idempotency key to bypass a failed/blocked Stripe result. Preserve the failed attempt evidence, complete the approved repayment outside Stripe only after Fraser signs off, then record the manual bank transfer through the ledger-only admin path with the bank reference, evidence/reference, and operator note.
...
- **Commission** (default): instructor gets `commission_rate` (e.g. 85%) of each lesson's net attributable revenue after its actual attributable processing fee
