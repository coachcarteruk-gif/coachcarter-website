...
The next implementation milestone must make account creation tenant-scoped, audit-logged and recoverable after ambiguous success; deliberately set Simon's `payouts_start_date`; start him with `payouts_paused=true`; and provide Fraser a read-only preview of exact lessons, approved Stripe funding, fees, the current admin-managed `weekly_franchise_fee_pence` deduction and proposed transfer. `chargeable` remains required, but Simon receives positive automatic value only where exact approved CoachCarter Stripe receipt is proven after the start boundary. Cash, external/Setmore, historic credit, pre-start, ambiguous and test sources remain manual or £0.
