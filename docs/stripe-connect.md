...
The implemented review boundary makes account creation tenant-scoped, audit-logged and recoverable after ambiguous success; deliberately sets `payouts_start_date`; starts the controlled instructor with `payouts_paused=true`; and provides Fraser a read-only preview of exact lessons, approved Stripe funding, fees, the current admin-managed `weekly_franchise_fee_pence` deduction and proposed transfer. `chargeable` remains required, but positive value exists only where exact approved CoachCarter Stripe receipt is proven after the start boundary. Cash, external/Setmore, historic credit, pre-start, ambiguous and test sources remain manual or £0.
