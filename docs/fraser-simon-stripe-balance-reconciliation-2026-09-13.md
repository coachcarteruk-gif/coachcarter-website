...
Deployment status: the preventive code and both dry-run-first repair runners were deployed through PR #460 on 13 September 2026 after the affected regression suite and repository CI passed. Migration 063 remains a separately governed database operation and has not been applied; it is required for Viba's same-booking replacement allocations, not for Megan's return-only repair. Viba's production repair remains unexecuted and separately approval-gated. None of these approvals authorizes Stripe changes, transfers, refunds, payout unpausing or any other production mutation.
