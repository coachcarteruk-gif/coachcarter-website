...
The default read-only runner was attempted locally, but the saved Neon credential is stale and failed authentication before any query ran. The deterministic preview below is generated by the tested planner from the confirmed production evidence above. After migration 063 is separately approved and applied, the runner must be rerun against a working read-only production connection; its live `ready` result and fingerprint must be reviewed before repair approval.

```json
{
  "mode": "dry-run",
  "mutation_performed": false,
  "status": "ready_after_migration_063",
...
| State | Before | After the separately approved repair |
|---|---:|---:|
...
The in-transaction postflight also requires: zero active units on refunded bookings; zero active allocation-duration mismatches; zero active Lesson Credit/Flexible Hours mixed-funding rows; 60 minutes of additive `admin_correction` evidence across credit transactions #334/#371; and the exact per-booking/source totals above. Those conditions are the database facts consumed by credit reconciliation, refund exposure, protected-balance and Simon payout previews. The operational postflight must still run each named read-only preview after the repair transaction and stop on any Viba contradiction.
...
Remaining limitation and approval boundary: migration 063, the preventive code and the repair runner are not deployed. The production state remains unchanged. A working read-only Neon credential is required to obtain the live preview. Deployment/migration approval and production-repair approval must remain separate; neither authorizes Stripe changes, transfers, refunds, payout unpausing or any other production mutation.
...
The **£729.50 gross/£725.36 net unclassified amount is pooled cash, not revenue**. It consists of Megan's £702 source-ledger remainder conflict and Emilie's £27.50 duration mismatch. Viba's former £189 unresolved amount is now proved to be unused package liability, although its production reclassification still requires separate approval.
...
- The production Neon evidence was refreshed through deployed, school-scoped read and preview endpoints rather than an interactive direct-SQL session because the current production integration does not expose a reusable database password locally. The endpoints executed fresh production queries, but unpublished raw columns cannot be independently queried through that route.
...
- Viba's production state remains unrepaired: ordinary `learner_credit_balances` incorrectly holds 210 minutes while the flexible source exposes only 90 minutes. The trace proves that the correct state is zero ordinary minutes and 300 flexible minutes, all valued from the original £810 package.
- Megan's live balance is 120 minutes while her flexible source ledger says 780 minutes remain. The exact fail-closed repair is implemented but not executed; until its production dry run confirms the allocation rows and the separately approved apply succeeds, the source cash remains unclassified.
- Viba #536/#568 remain technically contradictory until the proposed repair is executed, although their cause and correct two-unit values are now established. Emilie #533 remains unresolved, and #585 has only one hour of proven funding for a 90-minute future booking. These amounts are excluded from earned or protected commission beyond the supported duration.
