...
**2026-05-27 - `learner_users.credit_balance` is deprecated display state, not credit source of truth.**
Reason: After Phase 2A, real credit state lives in `learner_credit_balances.balance_minutes`, with `learner_users.balance_minutes` maintained as the pooled compatibility mirror by `trg_sync_pooled_balance`. A live refund/admin adjustment investigation found `credit_balance` can remain stale even when scoped and pooled minute balances are correct.
Consequence: New code must not use `credit_balance` for eligibility, balance display, or learner/instructor/admin messaging. Use scoped LCB where an instructor context matters, or minute-derived `balance_minutes` display for compatibility surfaces. Keep `credit_balance` only as deprecated API/database compatibility until a later cleanup.
