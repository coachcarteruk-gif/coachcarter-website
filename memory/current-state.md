...
- **Step 5 (BCS + FIFO) is in sliced rollout.** BCS/CSA schema substrate is present, and `booking_credit_sources.school_id` was applied on prod after PR #194. PR #211, PR #212, PR #213, PR #214, PR #216, PR #217, PR #218, PR #220, and PR #222 are merged. PR #216 shipped Step 5.5 `credit-goodwill`; PR #217 added the admin Learners UI affordance for granting goodwill credit; PR #218 shipped the backend-only `credit-reconciliation` writer behind `POST /api/admin?action=credit-reconciliation`; PR #220 shipped BCS attribution for paid, non-flexible, slot-pinned repeat offer series where every requested repeat week is successfully booked; PR #222 added pure read-only planner-only partial repeat accounting via `api/_bcs-repeat-offer-plan.js`. The admin UI remains inspection-only with no apply/grant path. Partial repeat offers remain intentionally outside the shipped live writer path until a later CSA-aware integration slice.
...
Post-PR-#222 production/current-state note: PR #222 "Plan partial repeat offer accounting" merged on 2026-05-26. It added planner-only partial repeat accounting via pure helper `api/_bcs-repeat-offer-plan.js::planPartialRepeatOfferAccounting` plus `tests/bcs-repeat-offer-plan.spec.js`. The helper proposes `booking_credit_sources` draw rows and `credit_source_adjustments` planner metadata for CSA/refund handling, but does not read/write the database and has no webhook wiring, admin UI/apply path, migrations, crons, Stripe calls, or production behavior change. No prod verification was required because there is no runtime wiring. No prod writes, migrations, payout crons, Neon/prod integration tests, Stripe mutations, goodwill/reconciliation grants, or admin grant paths were run.
