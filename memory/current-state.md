...
- **Credits divergence cron returns `drift_count = 0`; latest post-PR-#216 read-only output also has `missing_bcs_count = 0`.** verified by operator report 2026-05-26
...
- **Step 5 (BCS + FIFO) is in sliced rollout.** BCS/CSA schema substrate is present, and `booking_credit_sources.school_id` was applied on prod after PR #194. PR #211, PR #212, PR #213, PR #214, and PR #216 are merged. PR #216 shipped Step 5.5 `credit-goodwill`: admin-authenticated, scoped, zero-value goodwill grants through the shared LCB mutation path with audit action `admin.credit_goodwill_grant`. This branch adds the smallest admin Learners UI affordance for granting goodwill credit through that endpoint. `credit-reconciliation` remains contract-only / `NOT_IMPLEMENTED`. Remaining Step 5 scope is FIFO writer wiring and credit-reconciliation implementation.
...
| Step 5 — BCS + FIFO | SLICED ROLLOUT | Current `main` already has `booking_credit_sources`, `credit_source_adjustments`, BCS indexes, `UNIQUE (booking_id, credit_transaction_id)`, pence allocator tests, and read-only path-aware missing active BCS coverage detection in `api/cron-credit-reconcile.js`. Fraser accepted `booking_credit_sources.school_id` plus three writer-policy decisions on 2026-05-21. PR #194's BCS `school_id` migration was applied once on prod and post-diagnostic verified `school_id` exists/NOT NULL/default 1, FK + `idx_bcs_school` exist, null rows = 0, mismatch query = `[]`. PR #211, PR #212, PR #213, PR #214, and PR #216 are merged. PR #213 deployed the payout read model cutover to shared `getEligibleBookings`: prefer `lesson_bookings.list_price_pence`, use active BCS Stripe-fee sums when present, and exclude active instructor-absorbed BCS rows. PR #214 added focused read-only Next Payout Preview coverage proving those snapshotted gross and active BCS fee values flow through `computePlatformBalance()` / `simulatePayoutForInstructor()` preview totals. PR #216 shipped Step 5.5 `credit-goodwill`: validation requires reason, learner/instructor are checked inside the resolved school, the grant writes `type='admin_add'`, `source='goodwill'`, `amount_pence=0`, `stripe_fee_pence=0`, and `absorbed_by` via the shared `lockBalanceAndMutate` LCB path, then audit-logs `admin.credit_goodwill_grant`. This branch adds the Learners admin UI form for the shipped endpoint. `credit-reconciliation` remains contract-only / `NOT_IMPLEMENTED`. Remaining scope is FIFO writer wiring and credit-reconciliation implementation. |
...
- Last observed post-PR-#216 read-only output at 2026-05-26T06:13:57.369Z (operator report): `ok=true`, `schema_mode=full`, `has_bcs=true`, `has_bcs_school_id=true`, `has_csa=true`, `has_grandfathered_at=true`, `pairs_scanned=29`, `drift_count=0`, `missing_bcs_count=0`, `grandfathered_count=0`, `alert_sent=false`, `missing_bcs_truncated=false`, `drift_truncated=false`, `missing_bcs_summary=[]`, `drift_summary=[]`. No prod writes/migrations/backfills/payout crons/Neon integration tests/live Stripe calls/Stripe mutations were run. verified
