...
- **Step 5.5 contract-test slice in this branch:** `api/_admin-credit-contracts.js` pins the `credit-goodwill` and `credit-reconciliation` contracts, and `api/admin.js` exposes authenticated/validated stubs that return `NOT_IMPLEMENTED`. Tests cover auth-before-validation/no money mutation, goodwill scoping/positive minutes/absorber validation, expected goodwill ledger/audit shape, reconciliation identity lookup/no-op/idempotency, explicit Stripe reject conditions, and expected reconciliation audit action. The actual Stripe/DB writers remain unimplemented.
- **Post-PR-#213 prod verification:** PR #213 merged and deployed. Manual read-only `cron-credit-reconcile` trigger at 2026-05-25T22:15:04.183Z returned `ok=true`, `schema_mode=full`, `has_bcs=true`, `has_bcs_school_id=true`, `has_csa=true`, `has_grandfathered_at=true`, `pairs_scanned=29`, `drift_count=0`, `missing_bcs_count=0`, `grandfathered_count=0`, `alert_sent=false`, `missing_bcs_truncated=false`, `drift_truncated=false`, `missing_bcs_summary=[]`, and `drift_summary=[]`. No prod writes, migrations, backfills, payout crons, Neon integration tests, or Stripe mutations were run.
...
- **Likely files:** `api/_credit-grant.js` (writer wiring), `api/slots.js` / `api/webhook.js` / `api/offers.js` (allocation at booking time), `api/admin.js` + `api/_admin-credit-contracts.js` (Step 5.5 writers), `api/_payout-helpers.js` (cron + preview read per-source), `tests/booking-credit-sources.*`, `tests/admin-credit-contract.spec.js`.
- **Success criteria:**
