...
- **Likely files:** `api/_credit-grant.js` (writer wiring), `api/slots.js` / `api/webhook.js` / `api/offers.js` (allocation at booking time), `api/admin.js` + `api/_admin-credit-contracts.js` + `api/_admin-credit-goodwill.js` (Step 5.5 writers), `public/admin/portal.js` (goodwill UI), `api/_payout-helpers.js` (cron + preview read per-source), `tests/booking-credit-sources.*`, `tests/admin-credit-contract.spec.js`, `tests/admin-goodwill-ui.spec.js`.
- **Success criteria:**
