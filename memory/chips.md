...
- **Step 5.5 state in this branch:** `api/_admin-credit-contracts.js` pins the `credit-goodwill` and `credit-reconciliation` contracts. `credit-goodwill` is implemented via `api/_admin-credit-goodwill.js` + `lockBalanceAndMutate` with admin auth, school-scoped learner/instructor checks, reason validation, zero-value goodwill CT shape, LCB upsert/reconcile through the shared path, and audit action `admin.credit_goodwill_grant`. `credit-reconciliation` remains contract-only / `NOT_IMPLEMENTED`; no Stripe calls or reconciliation writer yet.
...
- **Likely files:** `api/_credit-grant.js` (writer wiring), `api/slots.js` / `api/webhook.js` / `api/offers.js` (allocation at booking time), `api/admin.js` + `api/_admin-credit-contracts.js` + `api/_admin-credit-goodwill.js` (Step 5.5 writers), `api/_payout-helpers.js` (cron + preview read per-source), `tests/booking-credit-sources.*`, `tests/admin-credit-contract.spec.js`.
- **Success criteria:**
