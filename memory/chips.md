...
- **Repeat-offer BCS slice shipped in PR #220:** PR #220 "Attribute BCS for fully booked repeat offers" merged and deployed on 2026-05-26. It shipped BCS attribution for paid, non-flexible, slot-pinned repeat offer series when every requested repeat week is successfully booked. The webhook splits the single `slot_purchase` credit transaction across all booked lessons using the existing BCS booking-plan helper, preserving exact conservation of minutes, contribution pence, and Stripe fee pence. Retry/idempotency is guarded by `ON CONFLICT (booking_id, credit_transaction_id) DO NOTHING`. Partial repeat offers remain intentionally outside this slice and need a later CSA-aware implementation. Focused local `npm.cmd test -- tests/bcs-fifo.spec.js tests/bcs-booking-plan.spec.js tests/webhook-offer-bcs.spec.js` passed: 26 passed.
- **Planner-only partial repeat-offer accounting slice shipped in PR #222:** PR #222 added `api/_bcs-repeat-offer-plan.js::planPartialRepeatOfferAccounting` plus `tests/bcs-repeat-offer-plan.spec.js`. It is a pure helper that proposes BCS draw rows plus `credit_source_adjustments` planner metadata for CSA/refund handling; it does not read/write the database, wire the webhook writer, add migrations, ship admin UI/apply paths, or change production behavior. Focused local `npm.cmd test -- tests/bcs-repeat-offer-plan.spec.js tests/bcs-fifo.spec.js tests/bcs-booking-plan.spec.js tests/webhook-offer-bcs.spec.js` passed: 35 passed. Partial repeats still need a later CSA-aware live integration slice.
- **Post-PR-#220 prod verification:** Manual read-only `cron-credit-reconcile` trigger at 2026-05-26T16:48:34.705Z returned `ok=true`, `schema_mode=full`, `has_bcs=true`, `has_bcs_school_id=true`, `has_csa=true`, `has_grandfathered_at=true`, `pairs_scanned=30`, `drift_count=0`, `missing_bcs_count=0`, `grandfathered_count=0`, `alert_sent=false`, `missing_bcs_summary=[]`, `drift_summary=[]`, `missing_bcs_truncated=false`, and `drift_truncated=false`. No prod writes other than the read-only cron trigger, migrations, payout crons, Neon/prod integration tests, live Stripe calls, Stripe mutations, goodwill grants, reconciliation grants, or UI apply/grant paths were run.
...
- **Likely files:** `api/_credit-grant.js` (writer wiring), `api/slots.js` / `api/webhook.js` / `api/offers.js` (allocation at booking time, including later CSA-aware handling for partial repeat offers), `api/admin.js` + `api/_admin-credit-contracts.js` + `api/_admin-credit-goodwill.js` + `api/_admin-credit-reconciliation*.js` (Step 5.5 admin grant/reconciliation paths), `public/admin/portal.js` (goodwill UI + reconciliation inspection-only UI), `api/_payout-helpers.js` (cron + preview read per-source), `tests/booking-credit-sources.*`, `tests/admin-credit-contract.spec.js`, `tests/admin-goodwill-ui.spec.js`, `tests/admin-credit-reconciliation-*.spec.js`, `tests/webhook-offer-bcs.spec.js`.
- **Success criteria:**
