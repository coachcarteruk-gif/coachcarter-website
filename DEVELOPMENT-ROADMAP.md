...
5. **`book.js` filter switches (follow-up, same day)**: switching instructor or lesson length re-fetched everything from scratch every time. Now a session cache keyed by `instructorId|lessonTypeId` (`feedCacheByFilter`) paints the previously loaded feed instantly (~10ms) and silently revalidates in the background so booked-out slots don't linger; lesson-type responses are cached per request URL the same way. The cache is invalidated (`resetFeedCaches()`) wherever slot state actually changes — booking, cancel, reschedule, postcode save, reschedule-mode entry. A `feedLoadToken` guard also fixes a latent race where a slow fetch from a previous filter could merge stale slots into the newer filter's view (fetches now merge into a per-run object, swapped in only if still current). `onFilterChange` no longer blocks the feed on `loadTestDateAvailability` (it renders its own panel).
