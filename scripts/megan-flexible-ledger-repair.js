...
/**
 * Approval-gated repair for the Megan Cridland Flexible Hours ledger incident.
 *
 * Dry-run is the default and performs read-only SQL only. Apply requires every
 * independent gate below, uses a direct database connection, and never calls Stripe.
 */
...
const MUTATION_ENV_GATE = 'MEGAN_FLEXIBLE_LEDGER_REPAIR_REVIEWED';
const APPLY_CONFIRMATION = 'APPLY_MEGAN_FLEXIBLE_LEDGER_REPAIR';
...
async function main() {
  ...
  const databaseUrl = mode === 'dry-run'
    ? (process.env.POSTGRES_URL_READONLY || process.env.POSTGRES_URL_UNPOOLED || process.env.POSTGRES_URL)
    : process.env.POSTGRES_URL_UNPOOLED;
  if (!databaseUrl) {
    throw new Error(mode === 'apply'
      ? 'POSTGRES_URL_UNPOOLED is required for apply mode'
      : 'POSTGRES_URL_READONLY, POSTGRES_URL_UNPOOLED or POSTGRES_URL is required');
  }

  if (mode === 'dry-run') {
    const preview = await buildMeganRepairPreview({
  ...
  if (process.env.MEGAN_FLEXIBLE_LEDGER_REPAIR_ENABLED !== MUTATION_ENV_GATE) {
    throw new Error(`Set MEGAN_FLEXIBLE_LEDGER_REPAIR_ENABLED=${MUTATION_ENV_GATE} for apply mode`);
  }
  ...
  try {
    ...
    const result = await applyMeganRepair(client, {
      reviewedFingerprint,
      adminId,
      operatorIdentity,
      evidenceReference,
