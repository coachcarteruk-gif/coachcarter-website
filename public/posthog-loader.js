...
(function () {
  ...
  function minimiseTrialEvent(event) {
    ...
    if (!event || ['trial_landing_viewed','trial_booking_cta_clicked','free_trial_page_viewed','free_trial_booking_started','free_trial_submitted','free_trial_confirmed'].indexOf(event.event) < 0) return null;
    var enums = { entry_page: ['test_booked','freetrial','free_direct','unknown'], campaign_key: ['test_booked_v1'], content_version: ['text_v1','video_v1'], form_version: ['test_details_v1'], placement: ['hero','no_test','after_intro','closing','sticky','general'] };
    var props = {};
  ...
  function postHogConfig() {
    ...
    if (isFreeConsultationPage() || isTrialFunnelPage()) {
      ...
      config.before_send = isTrialFunnelPage() ? minimiseTrialEvent : minimiseConsultationEvent;
  ...
  function checkAndLoad() {
    if (window.ccCookieConsent && window.ccCookieConsent.analyticsAllowed()) {
      loadPostHog();
    } else if (loaded) {
      disablePostHog();
    }
