...
| `STRIPE_RESERVED_BLOCK_BANK_PAYMENT_METHOD_CONFIGURATION` | Stripe Payment Method Configuration ID for Reserved Weekly Slot Pay by Bank-only Checkout |
...
- **Recurring weekly block foundation:** After an ordinary one-off booking, authenticated learners can preview matching same-instructor/day/time future weekly slots for the next 12 weeks. `recurring-block-preview` is read-only and stores nothing. `recurring-block-commit` supports full same-instructor Lesson Credit: it revalidates the selected future slots, creates a confirmed `recurring_slot_blocks` row plus booked item rows, creates all future `lesson_bookings`, writes BCS attribution, and decrements LCB in one transaction. `recurring-block-bank-checkout` supports the bank-payment hold path when same-instructor Lesson Credit is insufficient: it revalidates the selected future slots, creates a `pending_payment` block plus `held` items with a 10-minute expiry, creates a whole-block Stripe Checkout Session using the reserved-block bank payment configuration, stores Stripe refs, and returns the Checkout URL. The Stripe webhook converts paid bank holds into confirmed blocks, booked items, and `scheduled` `lesson_bookings`; failed or expired payments release held items while the block is still `pending_payment`. The bank path does not mutate LCB, write BCS rows, or create credit purchase rows. Partial credit, card/Apple Pay recurring block payment, notifications, eligible 48h+ bank-paid cancellation-to-credit policy, and expiry cron remain later slices.
...
| `reschedule` | POST | Yes | Move an ordinary scheduled one-off booking to a new slot (48hr+ notice, max 2 per chain, no balance change). Confirmed Reserved Weekly Slot occurrences are refused here and must use `reserved-policy-move`, so reserved moves remain on the reserved-slot-specific path. |
...
- **Stripe payment methods** — live Checkout uses dynamic payment methods. Klarna is being removed from Stripe configuration; do not reintroduce local Klarna copy or hardcoded Checkout method lists.
