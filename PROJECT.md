...
| `reschedule` | POST | Yes | Move an ordinary scheduled one-off booking to a new slot (48hr+ notice, max 2 per chain). Optional `new_instructor_id` switches the paid entitlement to another active same-school instructor without charging again; the server rechecks overlapping bookings, reservations, pending requests/offers, recurring holds, active availability, booking window, lesson-type support, transmission, and pickup travel before confirmation. Cross-instructor funding moves atomically through paired transfer ledger rows and replacement BCS, so the delivering instructor owns the later payout/credit-return scope. Body may include replacement `pickup_address` / `dropoff_address`. Confirmed Reserved Weekly Slot occurrences are refused here and must use `reserved-policy-move`, which remains same-instructor only. |
