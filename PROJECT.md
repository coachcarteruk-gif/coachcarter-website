...
- **Recurring weekly block foundation:** After an ordinary one-off booking, authenticated learners can preview matching same-instructor/day/time future weekly slots for the next 12 weeks. `recurring-block-preview` is read-only and stores nothing. `recurring-block-commit` supports full same-instructor Lesson Credit: it revalidates the selected future slots, creates a confirmed `recurring_slot_blocks` row plus booked item rows, creates all future `lesson_bookings`, writes BCS attribution, and decrements LCB in one transaction. `recurring-block-bank-checkout` supports the first bank-payment hold skeleton when same-instructor Lesson Credit is insufficient: it revalidates the selected future slots, creates a `pending_payment` block plus `held` items with a 10-minute expiry, creates a whole-block Stripe Checkout Session using the reserved-block bank payment configuration, stores Stripe refs, and returns the Checkout URL. It does not create `lesson_bookings`, mutate LCB, or write BCS rows. Partial credit, card/Apple Pay recurring block payment, webhook conversion, notifications, and expiry cron remain later slices.
...
| `recurring-block-bank-checkout` | POST | Learner | Reserved Weekly Slot bank checkout hold skeleton. Body: `{ anchor_booking_id, lessons }`. Rebuilds the preview server-side, requires all selected future slots to remain available, requires insufficient same-instructor Lesson Credit so the bank path is not a credit bypass, creates a `pending_payment` recurring block and `held` item rows with a 10-minute expiry, creates whole-block Stripe Checkout using `STRIPE_RESERVED_BLOCK_BANK_PAYMENT_METHOD_CONFIGURATION`, stores Stripe session/payment refs on the block, and returns `{ url }`. Does not create `lesson_bookings`, mutate `learner_credit_balances`, write BCS rows, convert payment success, send notifications, or run expiry cron. |
