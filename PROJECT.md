...
- **Spectator mode (April 2026; credit purchase update June 2026):** `/learner/book.html` is publicly accessible — every "Book" CTA across the marketing surface routes there directly (no login redirect). Logged-out visitors see a `#guestBanner` and a guest-aware sidebar (Upcoming and Profile filtered out via `authOnly`). The old `buy-credits.html` route is read-only for existing Lesson Credit balances, not a purchase page. Inside the booking modal, when the school's lesson types include a row with `slug='trial'`, an inline CTA "Claim this as your free trial →" redirects to `/free?instructor_id=…&date=…`. The slot is not force-converted; the trial handler enforces strict duration matching, so the guest re-picks a real trial slot on the dedicated page (which honours the hints by filtering and scrolling).
- **Free trial (self-serve, no payment):** Public `/free` page lets anyone book a 1-hour free first lesson without an account. The legacy `/free-trial.html` URL permanently redirects to `/free`. `/free-lesson.html` is the direct-response landing page for the funnel (July 2026 redesign) — every CTA on it points at `/free`; the booking and confirmation pages share its design system (Archivo/Instrument Sans, orange `#F58220`). A small display-only helper `free-trial-sticky.js` mirrors the slot summary into the mobile sticky submit dock. POSTs to `/api/slots?action=book-free-trial`, which creates a `scheduled` booking with `payment_method='free'`, `created_by='free_trial_self_serve'`, `minutes_deducted=0`. No Stripe involved. Confirmation directs the learner to existing six-digit email-code sign-in to manage the booking. Guarded against repeat use by email or phone (any status — cancelled trials count). Instructors must include `"trial"` in their `offered_lesson_types` (or have it `NULL`) to be surfaced. The page also accepts optional `?instructor_id=` and `?date=` hints (used by the `book.html` "claim as free trial" CTA) — `instructor_id` filters the slot feed, `date` highlights and scrolls to the matching day group via `.day-group--preselected`.
...
### Test-date free-trial funnel (21 September 2026; implemented, pending rollout)

`/test-booked` feeds `/free`. Strict school config `test_date_trial_funnel_enabled` gates the campaign, optional practical-test questions and immutable booking-time capture; default false. Migration 070 adds `trial_booking_intakes`, nullable `learner_users.test_booked` and `test_details_updated_at`. Existing test date/time/centre fields remain canonical. Public answers never overwrite an existing account. Authenticated Driving Test edits use a versioned, presence-aware patch; instructors see separately labelled current details and intake. Existing admin learner controls exposes the read-only `trial-funnel-report` action and aggregate CSV. VSL manifest is disabled; no watching is required. See [implementation, test evidence, report limits and exact rollout](docs/test-date-free-trial-funnel-implementation.md). No production rollout is authorised by this note.
...
| `set-availability` | POST | JWT | Update weekly availability windows. Body: `{ windows: [{ day_of_week, start_time, end_time, transmission_type? }], availability_conflict_token? }`. Newly uncovered existing bookings return `409 AVAILABILITY_BOOKING_CONFLICTS` with lesson details and a review token. Resubmit the same change with that token after acknowledgement; bookings stay intact. Replacement and old/new audit are atomic. |
...
| `delete-availability-override` | POST | JWT | Remove a one-off available window. Body: `{ id, availability_conflict_token? }`. Newly uncovered bookings require the same conflict-review acknowledgement as weekly availability. |
...
| `create-booking` | POST | JWT | Book a lesson on behalf of a learner (cash/credit/flexible-package/free payment). Schedule blocks and times outside recurring/one-off availability return `409 SCHEDULE_UNAVAILABLE`, regardless of legacy override/force fields. Add one-off availability or remove the block first. Flexible Hours records `created_by=instructor` for this route. Actual lesson and held-request conflicts are not overridable. |
...
**Availability enforcement (21 September 2026):** Instructor creation (all payment methods), fixed/broadcast offers, edits and extension requests require the entire lesson to fit weekly or one-off hours. Upcoming admin time edits use the same guard; completed-lesson corrections remain possible. Busy and external-calendar blocks cannot be overridden by a booking request. Weekly-hour reductions, new blackouts and one-off-window removal return newly uncovered existing lessons for explicit review before saving; they never cancel or move lessons. Instructor edits atomically store the old/new date, times, type, transmission and actor in `audit_log`, including admin impersonation. Historical override audit rows and existing bookings are preserved. See [availability policy](docs/instructor-availability-policy.md).
...
**`lesson_offers`** — instructor-initiated lesson offers pending learner acceptance and, when priced above zero, payment. Ordinary offers are **slot-pinned** or **flexible**; a booking-extension shape uses `extension_booking_id`, `extension_minutes`, and `extension_base_list_price_pence` to attach added time to an existing lesson. New offer pricing uses explicit `offer_price_pence` if supplied (including `0` for free), otherwise effective instructor pricing (custom learner rate → instructor hourly rate → school default); bulk tiers never apply. Paid Extension Checkout charges only the added minutes, and verified webhook fulfilment atomically appends a `slot_purchase` source and BCS row, extends the original booking end/minutes/payable value, and accepts the offer. Free extension acceptance bypasses Stripe and changes only the existing booking's end time plus offer status; it does not increase credit usage, refundable value or payout value. Partial unique indexes prevent duplicate pending fixed-slot and per-booking extension offers.
...
**Controlled authoritative payout evidence** — migration 061 adds append-only `payout_direct_evidence_observations`, `payout_flexible_source_evidence`, and versioned `payout_funding_basis_events`. `_authoritative-lesson-earning.js` calculates each commission lesson as the rounded share of net attributable revenue. Direct Stripe uses its exact gross/fee; Flexible Hours penny-conserves the original package gross and fee across 30-minute units; explicit audited manual/legacy semantics prevent invented prices and double commission. Simon's owner preview, instructor next-payout preview, calendar lesson lines, admin overview, approval and processor all consume `loadInterimV1Preview`. Migration 061 also adds nullable authoritative gross/fee/net and value-semantics fields beside the old non-null payout compatibility columns, so an audited net or already-final value is persisted without relabelling an unknown processing fee as zero. Missing evidence remains a visible blocker.
...
Admin support access can open an instructor portal session without password knowledge/reset via `POST /api/admin?action=access-instructor-account`. The resulting `cc_instructor` cookie is short-lived (2 hours), carries impersonation metadata, and is visibly marked in the instructor UI with a "Viewing as admin" banner. Admin-user sessions keep `cc_admin` intact. The session-persistence fix (23 September 2026, pending rollout) saves the original instructor JWT in an httpOnly `cc_instructor_return` cookie with its original expiry. Explicit support exit restores it even after the support cookie expires, after CSRF, signature, school and active-account checks, and returns canonical instructor display data. The saved cookie never authenticates ordinary API requests. Fresh instructor login and explicit instructor logout clear it. Legacy live support sessions retain their existing exit path during rollout. Instructor actions inside support still use normal instructor endpoints. Admin's Back to Portal only navigates; temporary verification failures preserve sessions.
...
| `access-instructor-account` | POST | Admin JWT | Starts support access for an active same-school instructor. Body: `{ instructor_id }`. Saves the original instructor session without extending expiry, mints a 2-hour support session and audit-logs `admin.instructor_access_start`. Does not mutate passwords. Nested support preserves the original saved login. |
| `stop-instructor-access` | POST | Admin, support or saved original instructor JWT + CSRF | Restores the valid original instructor cookie after an active-account lookup, clears the return cookie, refreshes CSRF and returns `{ success: true, instructor }` (null for admin-only return). Works after support expiry; cannot renew an expired original login. Audit-logs support exit/recovery. Clients update display state only after success and retain it on temporary failures. |
...
| `interim-v1-reconcile-funding-evidence` | POST | Superadmin | Exact-confirmation reconciliation for one controlled booking. Reads only already-recorded Stripe identities and appends idempotent direct/package evidence observations plus audit evidence. It cannot approve, claim, pay, unpause, refund, or transfer. |
| `interim-v1-record-funding-basis` | POST | Superadmin | Exact-confirmation, idempotent append-only classification for a controlled manual/external/legacy/correction basis. Requires explicit value semantics, reason and evidence reference; refuses claimed bookings and never calls Stripe or moves money. |
