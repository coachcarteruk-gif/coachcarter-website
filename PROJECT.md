...
| `create-extension-offer` | POST | Instructor JWT | Creates a 24-hour paid request for 30–180 additional minutes on an existing scheduled same-school lesson. Body: `{ booking_id, extension_minutes, offer_price_pence? }`. Server-priced unless a positive explicit price is supplied. Ignores availability windows and travel buffers, but blocks actual overlaps with bookings, busy blocks, pending offers/requests, and reservations. |
...
**`lesson_offers`** — instructor-initiated lesson offers pending learner acceptance + payment. Ordinary offers are **slot-pinned** or **flexible**; a booking-extension shape uses `extension_booking_id`, `extension_minutes`, and `extension_base_list_price_pence` to attach paid added time to an existing lesson. New offer pricing uses explicit `offer_price_pence` if supplied, otherwise effective instructor pricing (custom learner rate → instructor hourly rate → school default); bulk tiers never apply. Extension Checkout charges only the added minutes. Verified webhook fulfilment atomically appends a `slot_purchase` source and BCS row, extends the original booking end/minutes/payable value, and accepts the offer. Partial unique indexes prevent duplicate pending fixed-slot and per-booking extension offers.
